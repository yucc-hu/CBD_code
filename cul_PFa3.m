...
% 将G_task3数据reshape成842个173×12的cell数组
dataCellG = cell(numSubjects, 1);
for i = 1:numSubjects
    startIdx = (i - 1) * 173 + 1;
    endIdx = i * 173;
    dataCellG{i} = G_task3(startIdx:endIdx, :);
end

% 将V_task3数据reshape成842个173×12的cell数组
dataCellV = cell(numSubjects, 1);
for i = 1:numSubjects
    startIdx = (i - 1) * 173 + 1;
    endIdx = i * 173;
    dataCellV{i} = V_task3(startIdx:endIdx, :);
end
...
for i = 1:numConditions
    ...
    % 保存每个条件的结果到.mat文件
    save(['condition_' num2str(i) '_discrepancyCoefficientsant.mat'], 'conditionDiscrepancies');
end
