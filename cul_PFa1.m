...
% 将G_task1数据reshape成842个175×12的cell数组
dataCellG = cell(numSubjects, 1);
for i = 1:numSubjects
    startIdx = (i - 1) * 175 + 1;
    endIdx = i * 175;
    dataCellG{i} = G_task1(startIdx:endIdx, :);
end

% 将V_task1数据reshape成842个175×12的cell数组
dataCellV = cell(numSubjects, 1);
for i = 1:numSubjects
    startIdx = (i - 1) * 175 + 1;
    endIdx = i * 175;
    dataCellV{i} = V_task1(startIdx:endIdx, :);
end
...
for i = 1:numConditions
    ...
    % 保存每个条件的结果到.mat文件
    save(['condition_' num2str(i) '_discrepancyCoefficientsem.mat'], 'conditionDiscrepancies');
end
