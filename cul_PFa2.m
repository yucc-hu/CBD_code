...
% 将G_task2数据reshape成842个228×12的cell数组
dataCellG = cell(numSubjects, 1);
for i = 1:numSubjects
    startIdx = (i - 1) * 228 + 1;
    endIdx = i * 228;
    dataCellG{i} = G_task2(startIdx:endIdx, :);
end

% 将V_task2数据reshape成842个228×12的cell数组
dataCellV = cell(numSubjects, 1);
for i = 1:numSubjects
    startIdx = (i - 1) * 228 + 1;
    endIdx = i * 228;
    dataCellV{i} = V_task2(startIdx:endIdx, :);
end
...
for i = 1:numConditions
    ...
    % 保存每个条件的结果到.mat文件
    save(['condition_' num2str(i) '_discrepancyCoefficientsnb.mat'], 'conditionDiscrepancies');
end
